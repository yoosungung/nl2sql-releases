...
### 3.4 refSql inner table (warehouse-native escape hatch)

semantic SQL이 표현 못 하는 Postgres 구문(`LATERAL`, dialect 전용 함수 등)은 **같은 주제의 파생 model**에 `tables[].refSql`로 한 번 캡슐화한다. mcp가 unparse 시 `WITH <cte> AS (<refSql>)`로 인라인한다([nl2sql ARCHITECTURE §3.6](https://github.com/yoosungung/nl2sql/blob/main/ARCHITECTURE.md)).

**언제 쓰는가**

- per-row 서브쿼리·`CROSS JOIN LATERAL`이 질의 핵심인데 agent가 ANSI subset만 써야 할 때
- `tableReference`로는 grain·컬럼을 노출하기 어려운 전처리(정규화 signature, unnest 등)

**규칙**

- `refSql`과 `tableReference`는 **둘 중 하나만** (schema `innerTable` oneOf).
- SQL dialect = 해당 model의 `source`(여기서는 `local_postgres` → Postgres).
- `column.from`이 가리키는 alias·컬럼은 refSql 결과와 **일치**해야 한다(mcp는 refSql을 introspect하지 않음).
- 기존 단순 model은 유지하고, 파생 model을 추가하는 패턴을 권장(하위 호환).

**예: anagram signature** — `modern_data/modern_data_word_list_scored.model.json`

```json
{
  "name": "modern_data_word_list_scored",
  "source": "local_postgres",
  "description": "word_list — anagram signature. 단순 목록은 modern_data_word_list.",
  "tables": [
    {
      "alias": "w",
      "priority": 1,
      "refSql": "SELECT words AS word, (SELECT string_agg(ch, '' ORDER BY ch) FROM unnest(string_to_array(words, '')) AS ch) AS signature FROM modern_data.word_list"
    }
  ],
  "primaryKey": "word",
  "columns": [
    { "name": "word", "kind": "physical", "type": "text", "from": "w.word", "notNull": true },
    { "name": "signature", "kind": "physical", "type": "text", "from": "w.signature", "notNull": true }
  ]
}
```

agent semantic SQL 예 (LATERAL 없음):

```sql
SELECT word, (COUNT(*) OVER (PARTITION BY signature)) - 1 AS anagram_count
FROM modern_data_word_list_scored
WHERE length(word) BETWEEN 4 AND 5
ORDER BY word ASC
LIMIT 10
```
